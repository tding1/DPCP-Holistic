...
for i = 1:length(beta_list)
    ...
    dist_list{i} = l_RSGM(Xtilde, c, beta);
end
...
legend({'$\beta=0.1$', '$\beta=0.2$', '$\beta=0.4$', '$\beta=0.6$', '$\beta=0.8$', '$\beta=0.9$'}, 'FontSize', 20, 'Interpreter','LaTex')
...
%% local func
function dist_list = l_RSGM(X, c, beta)

    mu_0 = 1e-2; alpha = 1e-3; beta_ls = 1/2;
    maxiter = 100;
